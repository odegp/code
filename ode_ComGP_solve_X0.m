function [samptheta, samphyper, samplex0,SAMPLEX]= ode_ComGP_solve_X0(Data, ODE, GP, Iteration, X0)
...
for iter=1:Iteration.total
    iter
    %##############  Sampling Parameters #################################   
    % Since X0 is fixed, meany is fixed
    meany=repmat(xo,TTT,1); 
    ...
    for subiter=1:Iteration.sub 
        ...
        % Hence Mean of p(y|dotx,x0) is fixed now
        Mygxdot=dC'*inv(ddC);
        meandotx=Mygxdot*dotx+meany;
        
        % Hence Covaraince of p(y|dotx,x0) is now fixed except sigma^2*I
    ...
    for subiter=1:Iteration.sub 
        ...
        for DIM=1:size(y,2)
            ...
            for j=1:length(IT1)
                ...
                % Prior
                p3=feval(X0.prior{DIM},xo(DIM));
                
                              
                px0(j)=p1*p2*p3;
            end
            ind=randsample(length(IT1),1,true,px0);
            xo(DIM)=IT1(ind);
            clear IT1
            clear px0
        end
